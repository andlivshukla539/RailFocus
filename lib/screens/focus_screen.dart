...
const Map<String, _RouteVisuals> _kVis = {
  'tokyo_kyoto': _RouteVisuals(
    shortName: 'Tokyo → Kyoto', departure: 'TOKYO',   arrival: 'KYOTO',
    distanceKm: 513,  celestial: CelestialType.moon,
    isNight: true,    particle: ParticleKind.blossom,
    sky: [Color(0xFF0D0614), Color(0xFF1E0A22), Color(0xFF3D1535),
      Color(0xFF7A2E52), Color(0xFFB05E78)],
    farMtn: Color(0xFF110818), midMtn: Color(0xFF1A1025),
    hill:   Color(0xFF0E2415), tree:   Color(0xFF091A0F),
    ground: Color(0xFF060E08),
  ),
  'swiss_alps': _RouteVisuals(
    shortName: 'Zürich → Zermatt', departure: 'ZÜRICH', arrival: 'ZERMATT',
    distanceKm: 220, celestial: CelestialType.sun,
    isNight: false,  particle: ParticleKind.snow,
    sky: [Color(0xFF08111E), Color(0xFF0F2035), Color(0xFF1E3E60),
      Color(0xFF3A6A96), Color(0xFF7AAFC8)],
    farMtn: Color(0xFF1E2E3C), midMtn: Color(0xFF172636),
    hill:   Color(0xFF102E2A), tree:   Color(0xFF081E16),
    ground: Color(0xFF061410),
  ),
  'norwegian_fjords': _RouteVisuals(
    shortName: 'Oslo → Bergen', departure: 'OSLO', arrival: 'BERGEN',
    distanceKm: 490, celestial: CelestialType.moon,
    isNight: true,   particle: ParticleKind.firefly,
    sky: [Color(0xFF01040B), Color(0xFF030810), Color(0xFF060E1C),
      Color(0xFF0A1628), Color(0xFF0E1E36)],
    farMtn: Color(0xFF060E1A), midMtn: Color(0xFF081422),
    hill:   Color(0xFF061612), tree:   Color(0xFF030E0A),
    ground: Color(0xFF020808),
  ),
  'sahara_express': _RouteVisuals(
    shortName: 'Casa → Marrakech', departure: 'CASABLANCA', arrival: 'MARRAKECH',
    distanceKm: 240, celestial: CelestialType.sun,
    isNight: false,  particle: ParticleKind.sand,
    sky: [Color(0xFF100400), Color(0xFF281000), Color(0xFF5A2E00),
      Color(0xFFA86010), Color(0xFFD49A38)],
    farMtn: Color(0xFF4E300A), midMtn: Color(0xFF6A4414),
    hill:   Color(0xFF886020), tree:   Color(0xFF5A3808),
    ground: Color(0xFF3E2406),
  ),
};

_RouteVisuals _visuFor(RouteModel r) => _kVis[r.id] ?? const _RouteVisuals(
  shortName: 'Journey', departure: 'ORIGIN', arrival: 'DESTINATION',
  distanceKm: 300, celestial: CelestialType.sun, isNight: false,
...
class _FocusScreenState extends State<FocusScreen>
  ...
  void initState() {
    ...
    debugPrint('🚂 Focus: duration=${widget.durationMinutes}min, totalSec=$_totalSec');

    // Scenic controllers
    _scroll = AnimationController(vsync: this, duration: const Duration(seconds: 36))..repeat();
    _glow = AnimationController(vsync: this, duration: const Duration(seconds: 4))..repeat(reverse: true);
    _sway = AnimationController(vsync: this, duration: const Duration(milliseconds: 1400))..repeat(reverse: true);
    _cross = AnimationController(vsync: this, duration: const Duration(seconds: 14));
    ...
    // EQ bars
    _eqCtrl = AnimationController(vsync: this, duration: const Duration(milliseconds: 2200))..repeat();

    _fadein  = CurvedAnimation(parent: _enter, curve: Curves.easeOut);
    _slidein = Tween<Offset>(begin: const Offset(0, .04), end: Offset.zero)
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      _audio.playAmbientForRoute(_route.id);
  ...
  void _scheduleCross() {
    Future.delayed(Duration(seconds: 10 + math.Random().nextInt(12)), () {
      ...
      _cross.forward(from: 0).then((_) {
        if (mounted) { Future.delayed(const Duration(seconds: 18), _scheduleCross); }
      });
    });
  }
  ...
  void _startTimer() {
    _tick = Timer.periodic(const Duration(seconds: 1), (_) {
      ...
      if (_remSec <= 0) { _endSession(completed: true); }
    });
  }
  ...
  Future<void> _endSession({required bool completed}) async {
    ...
    await _storage.saveSession(JourneySession(
    ...
    if (mounted) {
      context.go(AppRouter.arrival, extra: {
        'route': _route,
        'mood': widget.mood,
        'goal': _goal,
        'durationMinutes': widget.durationMinutes ?? 25,
    ...
    return Scaffold(
      ...
      body: Stack(
        ...
        children: [
          ...
          const Positioned(
            top: 54, // Adjusted slightly to sit beneath the top edge of SafeArea
            right: 16,
      ...
      child: AnimatedBuilder(
        ...
        child: Container(
          decoration: BoxDecoration(
            ...
            border: Border.all(color: _C.copper.withValues(alpha: .48), width: 2.2),
            boxShadow: [
              BoxShadow(color: _activeAccent.withValues(alpha: .12),
                  blurRadius: 36, spreadRadius: 2),
              BoxShadow(color: Colors.black.withValues(alpha: .78),
          ...
          child: ClipRRect(
            ...
            child: Stack(
              ...
              children: [
                ...
                RepaintBoundary(child: AnimatedBuilder(
                  ...
                  builder: (_, __) => CustomPaint(
                    painter: _LandscapePainter(
                        vis: _vis, t: _scroll.value, paused: _paused),
                  ),
                )),
                RepaintBoundary(child: AnimatedBuilder(
                  animation: _scroll,
                  ...
                  child: IgnorePointer(child: DecoratedBox(
                    decoration: BoxDecoration(
                      gradient: LinearGradient(
                        ...
                        colors: [_C.bg.withValues(alpha: .30), Colors.transparent],
                      ),
                    ),
                  )),
                ),
                ...
                Positioned(bottom: 9, right: 12, child: Text(
                  _vis.shortName,
                  style: GoogleFonts.dmMono(
        ...
        return Column(children: [
          AnimatedDefaultTextStyle(
            ...
            style: GoogleFonts.cormorant(
              fontSize: 84, fontWeight: FontWeight.w200,
              letterSpacing: 10, height: 1.0,
              color: _paused
                  ? _C.cream.withValues(alpha: .30)
                  : _C.cream,
            ),
          ...
          Text(
            ...
            style: GoogleFonts.cormorant(
              ...
              color: _C.muted, letterSpacing: .6,
            ),
          ),
        ]);
      },
  ...
  Widget _buildProgressRail(Color accent) {
    return Padding(
      ...
      child: LayoutBuilder(builder: (_, cst) {
        ...
        return Column(children: [
          Row(children: [
            Text(_vis.departure, style: GoogleFonts.dmMono(
              fontSize: 7.5, color: _C.muted, letterSpacing: 1.6,
            )),
            const Spacer(),
            Text(_vis.arrival, style: GoogleFonts.dmMono(
              fontSize: 7.5, color: _C.muted, letterSpacing: 1.6,
            )),
          ]),
          ...
          SizedBox(
            ...
            child: ValueListenableBuilder<double>(
              ...
              builder: (_, p, __) => Stack(
                ...
                children: [
                  CustomPaint(size: Size(w, 22), painter: _TrackPainter()),
                  AnimatedContainer(
          ...
          ValueListenableBuilder<double>(
            ...
            builder: (_, p, __) => Row(
              ...
              children: [
                Text('${(p * _vis.distanceKm).round()} km',
                    style: GoogleFonts.dmMono(
                      fontSize: 10, fontWeight: FontWeight.w700,
                      color: accent, letterSpacing: 1,
                    )),
                Text('${_vis.distanceKm} km total',
                    style: GoogleFonts.dmMono(
                      fontSize: 9, color: _C.dim, letterSpacing: 1,
                    )),
              ],
            ),
          ),
    ...
    return Padding(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          color: playing
              ? _kAmbientColor.withValues(alpha: .10)
              : _C.card,
          border: Border.all(
            color: playing
                ? _kAmbientColor.withValues(alpha: .45)
                : _C.rim,
            width: playing ? 1.5 : 1,
          ),
          boxShadow: playing
                ...
                Row(children: [
                  ...
                  AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      ...
                      boxShadow: playing
                          ? [BoxShadow(
                        color: _kAmbientColor.withValues(alpha: .8),
                        blurRadius: 6,
                      )]
                          : null,
                    ),
            ...
            child: Row(
              ...
              children: [
                ...
                Text(
                  ...
                  style: GoogleFonts.dmMono(
                    fontSize: 10, fontWeight: FontWeight.w700,
                    color: _C.cream, letterSpacing: 2.8,
                  ),
                ),
              ],
            ),
...
class _StarFieldPainter extends CustomPainter {
  static final _stars = List.generate(65, (i) {
    ...
    return (x: r.nextDouble(), y: r.nextDouble() * .65,
...
class _SkyPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (vis.isNight) { _stars(canvas, size, _isLowEnd ? 40 : 80); }
    _celestial(canvas, size);
    if (vis.particle == ParticleKind.firefly) { _aurora(canvas, size); }
  }
  ...
  void _stars(Canvas canvas, Size size, int count) {
    ...
    for (int i = 0; i < count; i++) {
      canvas.drawCircle(
        Offset(rng.nextDouble() * size.width, rng.nextDouble() * size.height * .62),
        .5 + rng.nextDouble() * 1.1,
        Paint()..color = Colors.white.withValues(alpha: .18 + rng.nextDouble() * .58),
      );
    }
  }
  ...
  void _celestial(Canvas canvas, Size size) {
    ...
    final cy = size.height * .22 + math.sin(t * math.pi * 2) * size.height * .016;
      ...
      case CelestialType.sun:
        canvas.drawCircle(Offset(cx, cy), 62, Paint()
          ..shader = ui.Gradient.radial(Offset(cx, cy), 62, [
            const Color(0xFFFFE060).withValues(alpha: .16), Colors.transparent]));
        canvas.drawCircle(Offset(cx, cy), 38, Paint()
          ..shader = ui.Gradient.radial(Offset(cx, cy), 38, [
            const Color(0xFFFFEE80).withValues(alpha: .28), Colors.transparent]));
        canvas.drawCircle(Offset(cx, cy), 24, Paint()
          ..shader = ui.Gradient.radial(Offset(cx, cy), 24,
              [const Color(0xFFFFFDE8), const Color(0xFFFFD848),
      ...
      case CelestialType.moon:
        canvas.drawCircle(Offset(cx, cy), 44, Paint()
          ..shader = ui.Gradient.radial(Offset(cx, cy), 44,
              [Colors.white.withValues(alpha: .10), Colors.transparent]));
        canvas.drawCircle(Offset(cx, cy), 22, Paint()
          ..shader = ui.Gradient.radial(Offset(cx, cy), 22,
              [const Color(0xFFF5F3EF), const Color(0xFFCCCBC4)]));
        canvas.drawCircle(Offset(cx + 7, cy - 5), 5.0,
            Paint()..color = Colors.black.withValues(alpha: .07));
        canvas.drawCircle(Offset(cx - 6, cy + 6), 3.0,
...
class _LandscapePainter extends CustomPainter {
  ...
  const _LandscapePainter({required this.vis, required this.t, required this.paused});
  ...
  void _mtns(Canvas canvas, Size size, double t,
      double speed, double hFrac, double opacity, int seed, Color col) {
  ...
  void _hills(Canvas canvas, Size size, double t,
      double speed, double hFrac, int seed, Color col) {
    final rng = math.Random(seed);
    ...
    for (double x = -off - 90; x < size.width + 130; x += 48) {
      ...
      path.quadraticBezierTo(
        x + 24, y - 10 - rng.nextDouble() * 16,
        x + 48, y + rng.nextDouble() * 14,
      );
    }
  ...
  void _trees(Canvas canvas, Size size, double t, Color col) {
    ...
    for (double x = -off; x < size.width + 90; x += 30 + rng.nextDouble() * 22) {
  ...
  void _foreground(Canvas canvas, Size size, double t, Color col) {
    ...
    final wirePaint = Paint()
      ..color = col.withValues(alpha: .55) ..strokeWidth = 1.2 ..style = PaintingStyle.stroke;
    for (double x = -pOff; x < size.width + 110; x += 210) {
      canvas.drawRect(Rect.fromLTWH(x - 1.5, size.height - kGroundH - 44, 3, 44), polePaint);
      canvas.drawLine(Offset(x - 18, size.height - kGroundH - 39),
          Offset(x + 21, size.height - kGroundH - 39), wirePaint..strokeWidth = 2.2);
      canvas.drawLine(Offset(x + 19, size.height - kGroundH - 38),
          Offset(x + 192, size.height - kGroundH - 33), wirePaint..strokeWidth = 1.1);
      canvas.drawCircle(Offset(x - 17, size.height - kGroundH - 39), 2.0,
          Paint()..color = col.withValues(alpha: .70));
      canvas.drawCircle(Offset(x + 20, size.height - kGroundH - 39), 2.0,
          Paint()..color = col.withValues(alpha: .70));
    }
    final rPaint = Paint()..color = col.withValues(alpha: .55) ..strokeWidth = 2.5;
    canvas.drawLine(Offset(0, size.height - kRailTop),
        Offset(size.width, size.height - kRailTop), rPaint);
    canvas.drawLine(Offset(0, size.height - kRailTop - 6),
        Offset(size.width, size.height - kRailTop - 6), rPaint);
    final sOff   = (t * size.width * 8) % 24;
    final sPaint = Paint()..color = col.withValues(alpha: .30);
    for (double x = -sOff; x < size.width + 24; x += 24) {
      canvas.drawRect(Rect.fromLTWH(x - 1, size.height - kRailTop - 7, 2, 12), sPaint);
    }
  }
...
class _ParticlePainter extends CustomPainter {
  ...
  final double t;
  final bool paused;
  const _ParticlePainter({required this.vis, required this.t, required this.paused});

  @override
  void paint(Canvas canvas, Size size) {
    if (paused) { return; }
    switch (vis.particle) {
      case ParticleKind.blossom: _blossom(canvas, size); break;
      case ParticleKind.snow:    _snow(canvas, size);    break;
      case ParticleKind.firefly: _firefly(canvas, size); break;
      case ParticleKind.sand:    _sand(canvas, size);    break;
  ...
  void _blossom(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      final fall = (t * rng.nextDouble() * size.height * .6 + i * (size.height / 28)) % size.height;
      final px   = (bx + drft + t * 68) % (size.width + 30) - 15;
      final py   = (by + fall) % size.height;
      final r    = 2.0 + rng.nextDouble() * 2.5;
      p.color = const Color(0xFFFFB8C6).withValues(alpha: .25 + rng.nextDouble() * .45);
      canvas.save();
  ...
  void _snow(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      final fall  = (t * speed * size.height * 2.2 + i * (size.height / 36)) % (size.height + 40);
      final r  = 1.2 + rng.nextDouble() * 2.0;
      final sp = Paint()
        ..color = Colors.white.withValues(alpha: .35 + rng.nextDouble() * .45)
        ..strokeWidth = r * .55 ..style = PaintingStyle.stroke;
      canvas.save();
  ...
  void _firefly(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      canvas.drawCircle(Offset(px, py), r * 4, Paint()
        ..shader = ui.Gradient.radial(Offset(px, py), r * 4, [
          const Color(0xFF40D4BE).withValues(alpha: a), Colors.transparent]));
  ...
  void _sand(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      final px    = size.width -
          ((t * speed * size.width * 4 + i * (size.width / 55)) % (size.width + 60));
      final py    = by + math.sin(t * math.pi * 3 + i) * 6;
      final len   = 4.0 + rng.nextDouble() * 18;
      lp.color       = const Color(0xFFC8A050).withValues(alpha: .09 + rng.nextDouble() * .30);
...
class _CrossingTrainPainter extends CustomPainter {
  ...
  @override
  void paint(Canvas canvas, Size size) {
    if (progress <= .001 || progress >= .999) { return; }
  ...
  void _drawTrain(Canvas canvas, Size size) {
    ...
    final fireAlpha  = paused ? 0.12 : 0.12 + math.sin(t * math.pi * 2 * 6) * 0.06;
  ...
  void _drawCab(Canvas canvas, double fireAlpha) {
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(_cabX - _cabW, _cabTop, _cabW, _cabTop.abs() + _cabBot.abs()),
    ...
    canvas.drawCircle(
      ...
      Paint()
        ..shader = ui.Gradient.radial(
          Offset(_cabX - _cabW - 2, _cabBot - 10), 18,
    ...
    if (!paused) {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        canvas.drawLine(
          ...
          Offset(cxBase - cLen - 4 - 5 - math.sin(phase) * 3,
              cyBase + 3 + i * 2.5 - 4),
          wp,
        );
      }
  ...
  void _drawStack(Canvas canvas) {
    final tube = Path()
      ...
      ..quadraticBezierTo(
        _stackX - _stackR1 - 2, _stackBot - _stackH,
        _stackX - _stackR2,     _stackBot - _stackH,
      )
      ..lineTo(_stackX + _stackR2, _stackBot - _stackH)
      ..quadraticBezierTo(
        _stackX + _stackR1 + 2, _stackBot - _stackH,
        _stackX + _stackR1,     _stackBot - _stackH * .75,
      )
    ...
    canvas.drawRect(
      Rect.fromLTWH(_stackX - _stackR1 - 1, _stackBot - 4, (_stackR1 + 1) * 2, 4),
      Paint()..color = _brass.withValues(alpha: .70),
    );
  }
  ...
  void _drawHeadlamp(Canvas canvas) {
    ...
    canvas.drawCircle(
      ...
      Paint()
        ..shader = ui.Gradient.radial(
          Offset(lx - 3, ly - 1), 3,
          [Colors.white.withValues(alpha: .95), _win],
        ),
    );
    canvas.drawCircle(
      Offset(lx - 4, ly),
  ...
  void _drawFireglow(Canvas canvas, double alpha) {
    canvas.drawCircle(
      ...
      Paint()
        ..shader = ui.Gradient.radial(
          Offset(_cabX - _cabW - 5, -_drW * .3), 22,
  ...
  void _drawSpokedWheel(
    ...
    canvas.drawArc(
      Rect.fromCircle(
        center: Offset(cx + math.cos(cwA) * (r * .55), cy + math.sin(cwA) * (r * .55)),
        radius: r * .28,
      ),
      cwA - math.pi * .5,
      math.pi,
...
class _SpeedLinePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      final xOff = (t * size.width * 9 + i * size.width / 10) % (size.width + 90);
...
class _FramePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    canvas.drawRRect(
      ...
      Paint()..color = _C.copper.withValues(alpha: .11)
  ...
  void _corners(Canvas canvas, Size size) {
    final p = Paint()..color = _C.copper.withValues(alpha: .22)
    ...
    canvas.drawLine(Offset(size.width - d - l, d), Offset(size.width - d, d), p);
    canvas.drawLine(Offset(size.width - d, d), Offset(size.width - d, d + l), p);
    canvas.drawLine(Offset(d, size.height - d - l), Offset(d, size.height - d), p);
    canvas.drawLine(Offset(d, size.height - d), Offset(d + l, size.height - d), p);
    canvas.drawLine(Offset(size.width - d - l, size.height - d),
        Offset(size.width - d, size.height - d), p);
    canvas.drawLine(Offset(size.width - d, size.height - d - l),
        Offset(size.width - d, size.height - d), p);
  }
  ...
  void _rivets(Canvas canvas, Size size) {
    ...
    for (final o in [
      ...
      Offset(r, size.height - r), Offset(size.width - r, size.height - r),
    ]) {
      canvas.drawCircle(o, 4.5, Paint()
        ..shader = ui.Gradient.radial(
            o - const Offset(1.5, 1.5), 3.5, [_C.goldLt, _C.goldDk]));
      canvas.drawCircle(o, 4.5, Paint()
...
class _TrackPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double x = 0; x < size.width; x += 22) {
      canvas.drawRect(
          Rect.fromLTWH(x, (size.height - 11) / 2, 1.5, 11), tp);
    }
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: color.withValues(alpha: .65), blurRadius: 14)],
      ),
      child: const Center(
...
class _PressBtnState extends State<_PressBtn>
  ...
  late final Animation<double> _sc = Tween<double>(begin: 1.0, end: .94)
      ...
      child: ScaleTransition(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: const [
              BoxShadow(color: Color(0x28000000),
      ...
      child: Material(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(color: _C.danger.withValues(alpha: .08),
                  blurRadius: 48, spreadRadius: 4),
              BoxShadow(color: Colors.black.withValues(alpha: .72), blurRadius: 60),
            ],
          ),
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Container(
              ...
              child: const Icon(Icons.warning_amber_rounded, color: _C.danger, size: 34),
            ),
            const SizedBox(height: 22),
            Text('EMERGENCY STOP', style: GoogleFonts.cormorant(
              fontSize: 22, fontWeight: FontWeight.w700,
              color: _C.cream, letterSpacing: 2.5,
            )),
            const SizedBox(height: 10),
            Text(
            ...
            Row(children: [
              Expanded(child: _PressBtn(
                onTap: onCancel,
                child: Text('STAY ON', style: GoogleFonts.dmMono(
                  fontSize: 10, fontWeight: FontWeight.w700,
                  color: _C.cream, letterSpacing: 2.2,
                )),
              )),
              const SizedBox(width: 12),
              Expanded(child: _PressBtn(
                ...
                bg:     _C.danger,
                border: _C.danger,
                child: Text('END EARLY', style: GoogleFonts.dmMono(
                  fontSize: 10, fontWeight: FontWeight.w700,
